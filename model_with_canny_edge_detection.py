...
# Function to preprocess image using Canny Edge Detection + CLAHE
def load_and_preprocess_image(image_path):
    ...
    edges = cv2.Canny(gray, 100, 200)  # Apply edge detection

    # Convert edges to 3 channels so they can be blended with the original
    edges_colored = np.stack([edges] * 3, axis=-1)  

    # Blend edges with the original image (weighted sum)
    blended = cv2.addWeighted(image, 0.8, edges_colored, 0.2, 0)  # Adjust weights as needed

    # Convert to array format and normalize
    final_image = img_to_array(blended) / 255.0  
...
verification_model.save('iris_verification_model_With_Canny_edge_10_march_2025.h5', save_format='h5')
